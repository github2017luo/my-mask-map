...
var countyTown = document.querySelector('#countyTown');
var sideBarSwitchBtn = document.querySelector('#btnId');
...
(function () {
  ...
  request.onreadystatechange = function () {
    ...
    if (request.readyState === 4 && request.status === 200) {
      ...
      json.features.forEach(function (element) {return data.push(element);});
      var areaData = area.filter(
      function (element) {return element.CityName === selectedCounty;});

      // 預設未使用定位是會抓取 selectedCounty 資料
      // 移動至 selectedCounty 陣列第 0 筆 經緯度F
      var pharmacyData = data.filter(function (element) {return (
...
var sideBarOpenAndClose = function sideBarOpenAndClose() {
...
var changeCounty = function changeCounty(e) {
  // 已選擇城市
  selectedCounty = e.target.value;
  // 藥局資料庫不全使用外部資料 data.js
  var areaData = area.filter(function (element) {return element.CityName === selectedCounty;});
  var pharmacyData = data.filter(function (element) {return (
...
var changeTown = function changeTown(e) {
  // 預設選項第一個會是全部地區
  if (e.target.value === '全部地區') {
    ...
    // 已選擇 城市 + 已選擇 地區
    var countyAndTownStr = selectedCounty + e.target.value;
    var _pharmacyData = data.filter(function (element) {return (
        element.properties.address.match(countyAndTownStr));});

    // 參數帶入資料庫
    upDataSidebar(_pharmacyData);
    popupOpenOn(_pharmacyData);
  }
};
...
var clickBar = function clickBar(e) {
  if (e.target.id !== 'path') {
    return;
  }
  // 阻止元素默認的行為
...
"use strict";var addMarker = function addMarker(importData) {
  // marker群組管理
...
"use strict";var getDate = function getDate() {
  // DOM
...
"use strict";var getWeekAndIdCard = function getWeekAndIdCard() {
  // DOM
...
"use strict";var popupOpenOn = function popupOpenOn(importData) {
...
// }
"use strict";
"use strict";var upDataCounty = function upDataCounty(importData) {
  var str = '';
  importData.forEach(function (element) {
    str += "\n    <option value=\"".concat(
    element.CityName, "\">\n    ").concat(
...
"use strict";var upDataSidebar = function upDataSidebar(importData) {
  var str = '';
  importData.forEach(function (element) {
    var adultStockNoMore = function () {
    ...
    element.properties.name, "</h2>\n\t\t\t\t<a class=\"h1 h-text-primary fas fa-eye h-mr-3\" id=\"path\" href=\"#\"\n\t\t\t\tdata-lat = \"").concat(

    element.geometry.coordinates[1], "\"\n\t\t\t\tdata-lng = \"").concat(
    element.geometry.coordinates[0], "\"\n\t\t\t\tdata-name = \"").concat(
    element.properties.name, "\"\n\t\t\t\tdata-address = \"").concat(
    element.properties.address, "\"\n\t\t\t\tdata-phone = \"").concat(
    element.properties.phone, "\"\n\t\t\t\tdata-note = \"").concat(
    element.properties.note, "\"\n\t\t\t\tdata-adult = \"").concat(
    element.properties.mask_adult, "\"\n\t\t\t\tdata-child = \"").concat(
    element.properties.mask_child, "\"\n\t\t\t\t></a>\n\t\t\t\t<a class=\"fas fa-location-arrow\" href=\"https://www.google.com.tw/maps/dir//").concat(

    element.properties.address, "\" target=\"_blank\"></a>\n\t\t\t</div>\n\t\t\t<span class=\"h4 h-text-dark\">").concat(

    element.properties.address, "</span>\n\t\t\t<br>\n\t\t\t<span class=\"h4 h-text-dark\">").concat(

    element.properties.phone, "</span>\n\t\t\t<br>\n\t\t\t<span class=\"h4 h-text-dark\">").concat(

    element.properties.note, "</span>\n\t\t\t<br>\n\t\t\t<div class=\"h-d-flex h-mt-2\">\n\t\t\t\t<div class=\"p-badges ").concat(


    adultStockNoMore, "\"><span class=\"h5 h-flex-1\">\u6210\u4EBA\u53E3\u7F69</span><span>").concat(element.properties.mask_adult, "</span></div>\n\t\t\t\t<div class=\"p-badges ").concat(
    childStockNoMore, "\"><span class=\"h5 h-flex-1\">\u5152\u7AE5\u53E3\u7F69</span><span>").concat(element.properties.mask_child, "</span></div>\n\t\t\t</div>\n\t\t</div>\n\t\t");
...
"use strict";var upDataTown = function upDataTown(importData) {
  var str = "<option value=\"\u5168\u90E8\u5730\u5340\">\u5168\u90E8\u5730\u5340</option>";
  importData[0].AreaList.forEach(function (element) {
    str += "\n    <option value=\"".concat(
    element.AreaName, "\">\n    ").concat(
